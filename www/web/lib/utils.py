...
from settings import ROLE_FLAG_DICT
...
NAME_LASTNAME_REGEXP = "^[a-zA-Z-_ ]*$"
IMAGE_FILE_REGEXP = "^[^/\\]\.jpg$"
...
#TO convert unicode to string and strip trailing characters
def stringify(value):
...
def has_admin_access(user_info, entity):
  return user_has_role(user_info, 'business') and user_info.key in entity.owners

def replace_all_whitespace(input_string, delimiter='_'):
  return re.sub(r'\s+', delimiter, input_string).lower()